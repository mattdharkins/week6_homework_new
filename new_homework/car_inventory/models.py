...
class Car(db.Model):
    ...
    ext_color = db.Column(db.String(50), nullable = True)
    int_color = db.Column(db.String(50), nullable = True)
    odometer = db.Column(db.Integer, nullable = True)
    vin = db.Column(db.String(50))
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self,year,make,model, price,location,ext_color,int_color, odometer,vin,user_token, id = ''):
        self.id = self.set_id()
        self.year = year
        self.make = make
        self.model = model
...
class CarSchema(ma.Schema):
    class Meta:
        fields = ['id', 'year','make', 'model', 'price', 'location', 'ext_color', 'int_color', 'odometer', 'vin']
